...
void TraversImports(
    UAssetImportData& AssetImportData, std::unordered_set<std::string>& visited,
...
)
{
	...
	std::string str_path = filepath.lexically_normal().string();
	if (visited.find(str_path) != visited.end()) {
		return;
	}
	int id = visited.size();
	visited.insert(str_path);
